...
% All the constants that we will need
Root_AF = '2412';
Tip_AF = '0012';
b_3 = 33 + (4/12); % ft
c_r_3 = 5 + (4/12); % ft
...
% Recalculating liftslopes to be safe, as well as aerodynamic twist
...
% Calculating the lift slopes at the tip and the root, also defining N
a0_t_3 = a0_t_3 * 180/pi;
a0_r_3 = a0_r_3 * 180/pi;
N_3 = linspace(1,50,50);

% Running the PLLT function
for i = 1:length(N_3)
    [e_3(i), c_L_3(i), c_Di_3(i)] = PLLT(b_3, a0_t_3, a0_r_3, c_t_3, c_r_3, aero_t_3, aero_r_3, geo_t_3, geo_r_3, N_3(i));
end

% Calculating Errors
CL_err  = abs((c_L_3 - c_L_3(end)) ./ c_L_3(end)) * 100;
CDi_err = abs((c_Di_3 - c_Di_3(end)) ./ c_Di_3(end)) * 100;

% Setting thresholds
thresh = [10,1,0.1];

% Finding when each threshold is met and putting it into an array.
for i = 1:length(thresh)
    ...
    N_CDi_thresh(i) = N_3(idxCDi);
end

% Plotting CL vs N odd terms with all the thresholds plotted as well.
figure()
hold on;
grid on;
plot(N_3,c_L_3,'LineWidth',1.5,'Marker','.','MarkerSize',10)
xline(N_CL_thresh(1),'LineStyle',':','LineWidth',1.5)
xline(N_CL_thresh(2),'LineStyle','-.','LineWidth',1.5)
xline(N_CL_thresh(3),'LineStyle','--','LineWidth',1.5)
xlim([0 25])
legend('C_L as a function of N odd terms','10% Relative Error','1% Relative Error','0.1% Relative Error')
xlabel('Number of Odd Terms, N')
ylabel('Wing Lift Coefficient, C_L')
title('C_L Convergence vs. Number of Odd PLLT Terms')

% Plotting CDi vs N odd terms, again with thresholds plotted.
figure()
hold on;
grid on;
plot(N_3,c_Di_3,'LineWidth',1.5,'Marker','.','MarkerSize',10)
xline(N_CDi_thresh(1),'LineStyle',':','LineWidth',1.5)
xline(N_CDi_thresh(2),'LineStyle','-.','LineWidth',1.5)
xline(N_CDi_thresh(3),'LineStyle','--','LineWidth',1.5)
xlim([0 30])
legend('C_{Di} as a function of N odd terms','10% Relative Error','1% Relative Error','0.1% Relative Error')
xlabel('Number of Odd Terms, N')
ylabel('Wing Induced Drag Coefficient, C_{Di}')
title('C_{Di} Convergence vs. Number of Odd PLLT Terms')
...
CD_tot = cd_profile + c_Di_3(idxCDi);

% Defining constants found in the lab document
V_knot = 100;
V_cruise = 100 * 1.68781; % Ft/s
alt = 10000;
rho_10k = 0.001756;

% Finding Wing platform area and q_inf to calculate L and D
S = 0.5 * b_3 * (c_r_3 + c_t_3);
q_inf = 0.5 * rho_10k * V_cruise^2;

% Calcuating various L, D, and their ratio
ans.L = q_inf * S * c_L_3(idxCDi);
ans.Di = q_inf * S * c_Di_3(idxCDi);
D = q_inf * S * CD_tot;
ans.ratio = ans.L/D;

% Doing Deliverable 4 and 5
% This ones for the polyfits found later
AoA_poly = linspace(-12,12,13);
% This one is to be fed into the for loop calculating each CL and CDi
AoA_span = linspace(-12,12,50);

% Running the function for each point in the AoA_span
for i = 1:length(AoA_span)
    [e_AoA(i), c_L_AoA(i), c_Di_AoA(i)] = PLLT(b_3, a0_t_3, a0_r_3, c_t_3, c_r_3, aero_t_3, aero_r_3, 0 + AoA_span(i), 1 + AoA_span(i), 50);
end

% cd values for 0012 and 2412, found by visual inspection and comparing
% each graph in the Theory of Wings document.
cd_0012 = [0.013,0.0100,0.0085,0.0057,0.0065,0.006,0.0058,0.006,0.007,0.0075,0.0085,0.0105,0.013];
cd_2412 = [0.0125,0.0095,0.008,0.007,0.0067,0.0065,0.0063,0.0065,0.007,0.0075,0.009,0.011,0.014];

% Polyfitting and polyvaling for each airfoil
poly_0012 = polyfit(AoA_poly,cd_0012,2);
poly_2412 = polyfit(AoA_poly,cd_2412,2);
val_0012 = polyval(poly_0012,AoA_span);
val_2412 = polyval(poly_2412,AoA_span);

% Calculating cd_profile from the average of the root and tip, then
% calculating CD.
cd_profile_AoA = (val_2412 * c_r_3 + val_0012 * c_t_3) / (c_r_3 + c_t_3);
CD_AoA = cd_profile_AoA + c_Di_AoA;

% Plotting each CD component vs AoAspan
figure()
plot(AoA_span,CD_AoA,'LineWidth',1.5)
hold on;
grid on;
plot(AoA_span,c_Di_AoA,'LineWidth',1.5)
plot(AoA_span,cd_profile_AoA,'--','LineWidth',1.5)
title('Coefficent of Drag Components as a Function of Angle of Attack')
xlabel('Angle of Attack (degrees)')
ylabel('Coefficent of Drag Value')
xlim([-5 5])
ylim([0 0.025])
legend('Total Drag Coefficient (C_D)','Induced Drag Coefficient (C_{di})','Profile Drag Coefficient (C_d)','Location','northwest')

% Calculating L and D and L/D
L_AoA = q_inf * S * c_L_AoA;
D_AoA = q_inf * S * CD_AoA;
ratio_AoA = L_AoA./D_AoA;

% Plotting this ratio vs AoA_span
figure()
plot(AoA_span,ratio_AoA,'LineWidth',1.5)
hold on;
grid on;
title('L/D as a function of Angle of Attack')
xlabel('Angle of Attack (degrees)')
ylabel('L/D ratio')
legend('L/D Curve of Cessna Wing')
xlim([-2 12])
ylim([0 40])
